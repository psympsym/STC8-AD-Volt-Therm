...
#define UART1_GET_BUSY_FLAG()    	(UartBusyFlg & 0x01)
#define UART2_GET_BUSY_FLAG()    	(UartBusyFlg & 0x02)
#define UART3_GET_BUSY_FLAG()    	(UartBusyFlg & 0x04)
#define UART4_GET_BUSY_FLAG()    	(UartBusyFlg & 0x08)

/* 串口设置忙标志位宏函数，自定义的标志位变量，并非硬件寄存器 */
#define UART1_SET_BUSY_FLAG()    	do{UartBusyFlg |= 0x01;}while(0)
#define UART2_SET_BUSY_FLAG()    	do{UartBusyFlg |= 0x02;}while(0)
#define UART3_SET_BUSY_FLAG()    	do{UartBusyFlg |= 0x04;}while(0)
#define UART4_SET_BUSY_FLAG()    	do{UartBusyFlg |= 0x08;}while(0)

/* 串口清除忙标志位宏函数，自定义的标志位变量，并非硬件寄存器 */
#define UART1_CLEAR_BUSY_FLAG()   do{UartBusyFlg &= 0xFE;}while(0)
#define UART2_CLEAR_BUSY_FLAG()   do{UartBusyFlg &= 0xFD;}while(0)
#define UART3_CLEAR_BUSY_FLAG()   do{UartBusyFlg &= 0xFB;}while(0)
#define UART4_CLEAR_BUSY_FLAG()   do{UartBusyFlg &= 0xF7;}while(0)
